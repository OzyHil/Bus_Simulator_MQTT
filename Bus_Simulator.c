...
#define STATION_3 7
#define STATION_4 12
#define DISTANCE_POINT 15
#define DISTANCE_KILOMETER 3.4f

float proportional_distance = DISTANCE_KILOMETER / DISTANCE_POINT;
...
bool reversed = false; // Variável para controlar a direção do ônibus
...
// Publicar distâncias das estações
static void publish_stations(MQTT_CLIENT_DATA_T *state);
...
// Publicar distancias das estações
static void station_worker_fn(async_context_t *context, async_at_time_worker_t *worker);
static async_at_time_worker_t station_worker = {.do_work = station_worker_fn};
...
static void control_bus(MQTT_CLIENT_DATA_T *state, bool on)
{
    if (on)
    {
        if (reversed)
        {
            ...
            {
                reversed = false; // Inverte a direção quando chega ao início
                bus.posicao++;
            }
        }
        else
        {
            if (bus.posicao < DISTANCE_POINT)
                bus.posicao++;
            else
            {
                reversed = true; // Inverte a direção quando chega ao final
                bus.posicao--;
            }
        }
    }
    ...
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/bus/line"), station_line, strlen(station_line), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/bus/direction"), bus_direction, strlen(bus_direction), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}

// Publicar temperatura
static void publish_stations(MQTT_CLIENT_DATA_T *state)
{
    float distance_station_1 = abs(bus.posicao - STATION_1) * proportional_distance;
    float distance_station_2 = abs(bus.posicao - STATION_2) * proportional_distance;
    float distance_station_3 = abs(bus.posicao - STATION_3) * proportional_distance;
    float distance_station_4 = abs(bus.posicao - STATION_4) * proportional_distance;
    char distance_str[50];

    snprintf(distance_str, sizeof(distance_str), "%.2f km", distance_station_1);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/stations/station_1/bus_distance"), distance_str, strlen(distance_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);

    snprintf(distance_str, sizeof(distance_str), "%.2f km", distance_station_2);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/stations/station_2/bus_distance"), distance_str, strlen(distance_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);

    snprintf(distance_str, sizeof(distance_str), "%.2f km", distance_station_3);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/stations/station_3/bus_distance"), distance_str, strlen(distance_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);

    snprintf(distance_str, sizeof(distance_str), "%.2f km", distance_station_4);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/stations/station_4/bus_distance"), distance_str, strlen(distance_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
}
...
static void station_worker_fn(async_context_t *context, async_at_time_worker_t *worker)
{
    ...
    publish_stations(state);
    async_context_add_at_time_worker_in_ms(context, worker, TEMP_WORKER_TIME_S * 500);
}
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status)
{
    ...
    if (status == MQTT_CONNECT_ACCEPTED)
    {
        ...
        station_worker.user_data = state;
        async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &station_worker, 0);
    }
